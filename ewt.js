...
// @name         EWT Killer Box
// @namespace    EWTKILL
// @version      RELEASE 1.572
// @description 在使用之前,作者先劝各位学兄/弟们提个醒:能认真学习的尽量认真学习,只有认真学习才能获得一个好成绩!至于有什么功能？你用了就知道啦!
// @author       Sudo-su-Bash
// @connect      *
// @license      GPL2
...
const GET_LESSON_DETAIL_URL = "https://gateway.ewt360.com/api/homeworkprod/player/getLessonDetailV2";
...
const HOMEWORK_GET_ANSWER_URL = "https://web.ewt360.com/customerApi/api/studyprod/web/answer/webreport?reportId={reportid}&bizCode={bizCode}"
...
(function() {'use strict';$(function(){setTimeout(()=>{
    console.clear();
    console.log("开始初始化脚本...")
    ...
    function renderGuiOnListAndCourse() {
        ...
        let courseLstEle = $(".page-wrapper-3BCMS").length == 0 && $(".play_video_main_content_box").length == 0
        ...
        $("#ewt-toolbox-btn").click(async ()=> {
            renderBackground();
            await renderMainWindow();
            if(document.getElementsByClassName("play_video_main_box").length != 0) await drawWindowBody_Course();
    ...
    async function navComponent() {
        ...
        authorCol.append("<div class='kewt-title'>作者:Sudo-su-Bash</div>")
    ...
    async function drawWindowBody_Course() {
        ...
        let result = await getCourseInfo(courseInfo["lessonId"],basicUserInfo["data"]["schoolId"],courseInfo["homeworkId"])
        ...
        let courseName = $("<div class='kewt-course-col'>课程名:"+result["data"]["lessonName"]+"</div>");
        let courseSubject = $("<div class='kewt-course-col'>课程学科:"+result["data"]["subjectName"]+"</div>");
        let courseId = $("<div class='kewt-course-col'>课程id:"+result["data"]["courseId"]+"</div>");
        let courseDesc = $("<div class='kewt-course-col'>课程介绍:"+result["data"]["description"]+"</div>");
        let playTime = $("<div class='kewt-course-col'>完整播放时间:"+result["data"]["playTime"]+"</div>");
        ...
        jcBtn.mouseup(async function(){
            await JumpCourseClicked(jcBtn,courseInfo["lessonId"],courseInfo["courseId"],courseInfo["homeworkId"]);
        });
        ...
        addLog("kewt-logbox-0","登录的账户ID:"+basicUserInfo["data"]["userId"])
    ...
    async function drawWindowBody_Courses() {
        ...
        for(let i=0;i<dayinfo["data"]["days"].length;i++) {
            ...
            s.click(async ()=>{
                ...
                for(let j = 0;j < missions["data"]["data"].length;j++) {
                    let lessonid = getUrlInfo(missions["data"]["data"][j]["contentUrl"])["lessonId"];
                    let courseid = getUrlInfo(missions["data"]["data"][j]["contentUrl"])["courseId"]
                    let homeworkid = getUrlInfo(missions["data"]["data"][j]["contentUrl"])["homeworkId"]
        ...
        wholeFunc.append($("<div class='kewt-common-btn btn-red' id='jc-whole-btn'><label>点击跳过当天的全部课程</label></div>"));
        wholeFunc.append($("<div class='kewt-common-btn btn-green' id='fo-whole-btn'><label>点击填充当天全部练习的选择题</label></div>"))
        ...
        function drawQuestions(parentContainer,questionList) {
            ...
            for(let i of questionList) {
                ...
                if(i["options"].length != 0) {
                    ...
                    answerContainer.append("<div class='ques-opt-container'>"+
                        (i["childQuestions"].length==0 ? "答案": "小题部分")+"</div>")
    ...
    async function JumpDayCourseClicked(missions,homeworkId) {
        ...
        for(let j = 0;j < missions["data"]["data"].length;j++) {
            lessonid = getUrlInfo(missions["data"]["data"][j]["contentUrl"])["lessonId"];
            ...
            if(missions["data"]["data"][j]["contentTypeName"] == "课程讲") {
                let courseInfo = await getCourseInfo(lessonid,basicUserInfo["data"]["schoolId"],phomeworkid)
                addLog("kewt-logbox-0","==================")
                addLog("kewt-logbox-0","开始跳课,课程名:"+courseInfo["data"]["lessonName"])
    ...
    async function FillOptionBtnClicked(fhBtn,homeworks,bizCode) {
        ...
        for(let i=0;i<homeworks.length;i++) {
            let paperid = null;
            addLog("kewt-logbox-0","")
            addLog("kewt-logbox-0","开始自动填充第 "+(i+1)+" 份(共 "+homeworks.length+" 份)作业/卷的选择题...")
            ...
            if(await FillOptionBtnEvent(paperid,bizCode)) {
                addLog("kewt-logbox-0","第 "+(i+1)+" 份作业/试卷选择题填写完毕.");
            }
            else {
                addLog("kewt-logbox-0","第"+(i+1)+"份作业/试卷选择题填写失败.程序终止.");
    ...
    async function FillDayOptionBtnClicked(missions,homeworkId) {
        ...
        addLog('kewt-logbox-0',"=== 开始填写当天课程附带练习/试卷上所有的选择题...")
        addLog('kewt-logbox-0','使命任务ID:'+homeworkId);
    ...
    async function FillOptionBtnEvent(paperId,bizCode) {
        ...
        addLog("kewt-logbox-0","试卷/作业标题:"+paperContent["data"]["title"])
        addLog("kewt-logbox-0","试卷/作业ID:"+paperContent["data"]["paperId"])
        let res = await fillOption(paperId,reportId,finalAnswer,bizCode);

        if(res["success"])
            addLog("kewt-logbox-0","自动填写选择题完成.")
        else
            addLog("kewt-logbox-0","填充选择题失败(错误代码:"+res["code"]+",错误原因:"+res["msg"]+")")
        return res["success"];
    }
    ...
    async function finishCourse(lessonid,courseId) {
        let vid = await getCourseInfo(lessonid,basicUserInfo["data"]["schoolId"],courseId);
    ...
    class getSignature {
        constructor(videoTime) {
            this.begin = Date.now();
            this.duration = (parseInt(videoTime.split(":")[0]) * 60 + parseInt(videoTime.split(":")[1]) -45) * 1000;
            this.param = "action=4&duration={duration}&mstid={mstid}&signatureMethod=HMAC-SHA1&signatureVersion=1.0&timestamp={timestamp}&version=2022-08-02";
        ...
        async getRealSignature() {
            ...
            let rp = this.param.replace("{duration}",this.duration*1000)
    ...
    async function finishCourseFn(lessonid,homeworkId,courseid) {
        ...
        if(res["success"] != true) {
            addLog("kewt-logbox-0","抱歉,跳课失败(错误代码:"+res["code"]+",错误原因:"+res["msg"]+").")
